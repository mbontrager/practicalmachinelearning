...
We can remove all rows which contain summary statistics and not observational data. We can also filter out all columns which serve only to present summary data. We can also remove the "X" variable, which is just the row number and the "new_window" variable which was only a marker for summary variable rows.
...
This doesn't look too bad, but it also doesn't really look great and I think I could do a lot better. I don't want to check this on the test set yet because I think I can do better just by using a random forests model on the training set.
